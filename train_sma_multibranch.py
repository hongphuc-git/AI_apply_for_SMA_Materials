...
class SMAMultiBranchConfig(SMAAnnConfig):
    ...
    clk_head_hidden: tuple[int, ...] = (96, 48)
...
class SMAMultiBranchNet(nn.Module):
    def __init__(self, config: SMAMultiBranchConfig) -> None:
        ...
        if self.reverse_onset_points <= 1 or self.reverse_onset_points > self.unloading_points:
        ...
        self.reverse_onset_branch = make_mlp(config.reverse_onset_points + 5, config.reverse_onset_hidden, config.dropout)
        self.rate_branch = make_mlp(8, config.rate_hidden, config.dropout)
        ...
        reverse_onset_out = config.reverse_onset_hidden[-1] if config.reverse_onset_hidden else (config.reverse_onset_points + 5)
        rate_out = config.rate_hidden[-1] if config.rate_hidden else 8
        fusion_in = loading_out + unloading_out + hysteresis_out + reverse_onset_out + rate_out
        ...
        clk_layers: list[nn.Module] = []
        clk_in = current
        for width in config.clk_head_hidden:
            clk_layers.extend(
                [
                    nn.Linear(clk_in, width),
                    nn.BatchNorm1d(width),
                    nn.GELU(),
                    nn.Dropout(config.dropout),
                ]
            ...
            clk_in = width
        clk_layers.append(nn.Linear(clk_in, 3))
        self.clk_head = nn.Sequential(*clk_layers)

        asd_layers: list[nn.Module] = []
        asd_in = current
        for width in config.asd_head_hidden:
            asd_layers.extend(
                [
                    nn.Linear(asd_in, width),
                    nn.BatchNorm1d(width),
                    nn.GELU(),
                    nn.Dropout(config.dropout),
                ]
            ...
            asd_in = width
        asd_layers.append(nn.Linear(asd_in, 1))
        self.asd_head = nn.Sequential(*asd_layers)
    ...
    def build_meta_features(loading: torch.Tensor, unloading: torch.Tensor, rate: torch.Tensor) -> torch.Tensor:
        ...
        hysteresis_gap = torch.abs(loading - unloading_reversed)
        ...
        loop_area_proxy = hysteresis_gap.mean(dim=1, keepdim=True)
    ...
    def build_reverse_onset_features(self, loading: torch.Tensor, unloading: torch.Tensor) -> torch.Tensor:
        ...
        onset_gap_last = torch.abs(loading[:, self.reverse_onset_points - 1 : self.reverse_onset_points] - onset_ref[:, self.reverse_onset_points - 1 : self.reverse_onset_points])
        return torch.cat([onset, onset_mean_slope, onset_max_slope, onset_drop, onset_gap_mean, onset_gap_last], dim=1)
    ...
    def forward(self, features: torch.Tensor) -> torch.Tensor:
        ...
        clk = self.clk_head(trunk)
        asd = self.asd_head(trunk)
        output = torch.cat([clk, asd], dim=1)
        return self.output_sigmoid(output)
